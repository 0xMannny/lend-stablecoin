...
def mintTo(_to: address, _value: uint256) -> bool:
    ...
    """
    # Run Checks
    assert msg.sender == self.minter or msg.sender == self.controller
    # Update Values
...
def burnFrom(_from: address, _value: uint256) -> bool:
    ...
    """
    # Run Checks
    assert msg.sender == self.minter or msg.sender == self.burner or msg.sender == self.controller
    # Update Values
    self.totalSupply -= _value
    self.balances[_from] -= _value
    # Log Transfer
...
@external
def setMinter(_to: address):
    assert msg.sender == self.minter
    self.minter = _to


@external
def setBurner(_to: address):
    assert msg.sender == self.burner
    self.burner = _to


@external
def setController(_to: address):
